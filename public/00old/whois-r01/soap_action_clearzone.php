...
if ( (isset($_POST['login']) && isset($_POST['password'])) && (!empty($_POST['login']) && !empty($_POST['password'])) )
{
   ...
   {  
      // вызываем функцию
      $delallrrrecord = $client->clearZone($_POST['domain']); 
   }
   catch(SoapFault $fault)
   {
      // Не удалось вызвать функцию clearZone на сервере или она отработала неправильно.
      echo $format1."Couldnt execute clearZone".$format2;
      echo "Fault code: ".$fault->faultcode."<br>Fault message: ".$fault->faultstring;
      logout();
      exit();
   }
   if ($delallrrrecord->status->code != '1')
   {
      // функция отработала, возникла обработанная ошибка с идентификатором $delallrrrecord->status->name, выдаем сообщение.
      echo $format1."Failed to clear zone ".$format2;
      echo "Error name: ".$delallrrrecord->status->name." <br>Error message: ".$delallrrrecord->status->message;
      logout();
      exit();
   }
   else
   {
      // Успешно
      echo "<br><b>clearZone status:</b> ".$delallrrrecord->status->name;
      echo "<br><b>clearZone message:</b> ".$delallrrrecord->status->message;
   }
