...
if ( (isset($_POST['login']) && isset($_POST['password'])) && (!empty($_POST['login']) && !empty($_POST['password'])) )
{
   ...
   try 
   {  
      // вызываем функцию фиксании абонента   
      $mask = $client->masqueradeAs($_POST['agr_number']);
   }
   catch(SoapFault $fault)
   {
      // Не удалось вызвать функцию masqueradeAs на сервере или она отработала неправильно.
   ...
   {
      // функция отработала, возникла обработанная ошибка с идентификатором $mask->status->name, выдаем сообщение.
   ...
   {
      // Успешно зафиксировали абонента
