...
if ( (isset($_POST['login']) && isset($_POST['password'])) && (!empty($_POST['login']) && !empty($_POST['password'])) )
{
   ...
   if ( isset($_POST['nic_hdl']) && isset($_POST['domain_name']) && isset($_POST['accounttype']) )
   {
      ...
      // Внимание, все передаваемые через SOAP данные должны быть в кодировке UTF-8, 
      // поэтому все русские символы необходимо явно конвертировать в UTF-8.
      try 
      {  
      ...
      {
         // Не удалось вызвать функцию addDomainRfRequest на сервере или она отработала неправильно.
      ...
      {
         // функция отработала, возникла обработанная ошибка с идентификатором $addRequest->status->name, выдаем сообщение.
