...
if ( (isset($_POST['login']) && isset($_POST['password'])) && (!empty($_POST['login']) && !empty($_POST['password'])) )
{
   ...
   if ( isset($_POST['id']) && !empty($_POST['id'] ))
   {
      try 
      {  
         // вызываем функцию редактирования RR записи   
         $data = array(
      ...
      {
         // Не удалось вызвать функцию editRrRecord на сервере или она отработала неправильно.
      ...
      {
         // функция отработала, возникла обработанная ошибка с идентификатором $editrecord->status->name, выдаем сообщение.
      ...
      print_r($editrecord->status->message);
         // RR запись успешно отредактирована.
