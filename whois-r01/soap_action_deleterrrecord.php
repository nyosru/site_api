...
if ( (isset($_POST['login']) && isset($_POST['password'])) && (!empty($_POST['login']) && !empty($_POST['password'])) )
{
   ...
   if ( isset($_POST['id']) && !empty($_POST['id'] ))
   {
      try 
      {  
         // вызываем функцию удаления RR записи   
         $deleterecord = $client->deleteRrRecord($_POST['id']); 
      }
      catch(SoapFault $fault)
      {
         // Не удалось вызвать функцию deleteRrRecord на сервере или она отработала неправильно.
         echo $format1."Couldnt execute deleteRrRecord".$format2;
         echo "Fault code: ".$fault->faultcode."<br>Fault message: ".$fault->faultstring;
         logout();
         exit();
      }
      if ($deleterecord->status->code != '1')
      {
         // функция отработала, возникла обработанная ошибка с идентификатором $deleterecord->status->name, выдаем сообщение.
         echo $format1."Failed to delete RR record ".$format2;
         echo "Error name: ".$deleterecord->status->name." <br>Error message: ".$deleterecord->status->message;
         logout();
         exit();
      }
      else
      {
      print_r($deleterecord->status->message);
         // RR запись успешно удалена
         echo "<br><b>deleteRrRecord status:</b> ".$deleterecord->status->name;          
         echo "<br><b>deleteRrRecord message:</b> ".$deleterecord->status->message;         
      }
   }
   else
   {
      // пустой домен или ид записи
