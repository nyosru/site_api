...
if ( (isset($_POST['login']) && isset($_POST['password'])) && (!empty($_POST['login']) && !empty($_POST['password'])) )
{
   ...
   try 
   {  
      // вызываем функцию получения списка дочерних абонентов   
      $abonents = $client->getAbonents();
   }
   catch(SoapFault $fault)
   {
      // Не удалось вызвать функцию getAbonents на сервере или она отработала неправильно.
      echo $format1."Couldnt execute getAbonents".$format2;
      echo "Fault code: ".$fault->faultcode."<br>Fault message: ".$fault->faultstring;
      logout();
      exit();
   }
   if (is_object($abonents))
   {
      if ($abonents->status->code != '1')
      {
         // функция отработала, возникла обработанная ошибка с идентификатором $abonents->status->name, выдаем сообщение.
         echo $format1."Failed to get abonents ".$format2;
         echo "Error name: ".$abonents->status->name." <br>Error message: ".$abonents->status->message;
         logout();
         exit();
      }
      else
      {
         // Успешно получили список абонентов.
         echo "<br><b>getAbonents status:</b> ".$abonents->status->name;
         echo "<br><b>getAbonents message:</b> ".$abonents->status->message;
         echo "<table border=1><tr><td>N</td><td>Parent agr. number</td><td>Agreement number</td></tr>";
         foreach($abonents->data as $key => $abonent)
         {
            echo "<tr><td>".($key+1)."</td>
            <td>".$abonent->parent_agr_number."</td>
            <td>".$abonent->agr_number."</td></tr>";
         }
         echo '</table>';
      }
   }
