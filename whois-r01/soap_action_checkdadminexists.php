...
if ( (isset($_POST['login']) && isset($_POST['password'])) && (!empty($_POST['login']) && !empty($_POST['password'])) )
{
   ...
   try 
   {  
      // вызываем функцию проверки существования Администратора домена       
      $check_dadmin_ex = $client->checkDadminExists($_POST['nic_hdl']);
   }
   catch(SoapFault $fault)
   {
      // Не удалось вызвать функцию checkDadminExists на сервере или она отработала неправильно.
      echo $format1."Couldnt execute checkDadminExists".$format2;
      echo "Fault code: ".$fault->faultcode."<br>Fault message: ".$fault->faultstring;
      logout();
      exit();
   }
   if ($check_dadmin_ex->status->code != '1')
   {
      // функция отработала, возникла обработанная ошибка с идентификатором $check_dadmin_ex-->status->name, выдаем сообщение.
      echo $format1."Failed to check dadmin information ".$format2;
      echo "Error name: ".$check_dadmin_ex->status->name." <br>Error message: ".$check_dadmin_ex->status->message;
      logout();
      exit();
   }
   else
   {
      // Успешно проверили существование.
      echo "<br><b>checkDadminExist status:</b> ".$check_dadmin_ex->status->name;          
      echo "<br><b>checkDadminExist message:</b> ".$check_dadmin_ex->status->message;         
   }
   logout();   
}
else
