...
if ( (isset($_POST['login']) && isset($_POST['password'])) && (!empty($_POST['login']) && !empty($_POST['password'])) )
{
   ...
   if ( isset($_POST['domain']) && isset($_POST['type_record']) && !empty($_POST['domain']) && !empty($_POST['type_record']))
   {
      ...
      {  
         ...
         $addnewrecord = $client->addNewRrRecordAsync($_POST['domain'], $_POST['type_record'], $data ); 
      }
      catch(SoapFault $fault)
      {
         // Не удалось вызвать функцию addNewRrRecordAsync на сервере или она отработала неправильно.
         echo $format1."Couldnt execute addNewRrRecordAsync".$format2;
         echo "Fault code: ".$fault->faultcode."<br>Fault message: ".$fault->faultstring;
         logout();
         exit();
      }
      ...
      else
      {
         ...
         echo "<br><b>addNewRrRecordAsync status:</b> ".$addnewrecord->status->name;          
         echo "<br><b>addNewRrRecordAsync message:</b> ".$addnewrecord->status->message;         
         echo "<br><b>addNewRrRecordAsync Task ID:</b> ".$addnewrecord->taskid;         
      }
   }
   else
   {
