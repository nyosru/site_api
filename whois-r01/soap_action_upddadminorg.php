...
if ( (isset($_POST['login']) && isset($_POST['password'])) && (!empty($_POST['login']) && !empty($_POST['password'])) )
{
   ...
   if (isset($_POST['nic_hdl']))
   {
      ...
      {         
         ...
         $updatedadmin_result = $client->updateDadminOrg( $_POST['nic_hdl'],    iconv("KOI8-R", "UTF-8", $_POST['orgname_ru']),
                                                       $_POST['orgname_en'], $_POST['inn'], $_POST['kpp'], $_POST['ogrn'],
                                                       iconv("KOI8-R", "UTF-8", $_POST['legal_addr']), 
                                                       iconv("KOI8-R", "UTF-8", $_POST['postal_addr']), 
                                                       $_POST['phone'],  $_POST['fax'], $_POST['e_mail'],
                                                       iconv("KOI8-R", "UTF-8", $_POST['director_name']), 
                                                       iconv("KOI8-R", "UTF-8", $_POST['bank']), $_POST['ras_schet'],
                                                       $_POST['kor_schet'], $_POST['bik'],$_POST['isresident']
                                                     ); 
      }
      catch(SoapFault $fault)
      {
         // Не удалось вызвать функцию updateDadminOrg на сервере или она отработала неправильно.
         echo $format1."Couldnt execute updateDadminOrg".$format2;
         echo "Fault code: ".$fault->faultcode."<br>Fault message: ".$fault->faultstring;
         logout();
         exit();
      }
      ...
      {
         // функция отработала, возникла обработанная ошибка с идентификатором $updatedadmin_result->status->name, выдаем сообщение.
      ...
      {
         // Успешно отредактировали aдминистратора.
         echo "<br><b>updateDadminOrg message:</b> ".$updatedadmin_result->status->message; 
   ...
   logout(); // все сделали, разлогинимся.
}
else
{
