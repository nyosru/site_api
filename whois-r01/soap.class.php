...
class ssoap
{
   ...
   private static $soap_server_address = 'https://cp.centrohost.ru:1443/partner_api.khtml';
   ...
   public static function start()
   {
      ...
      try {
         self::$client = new SoapClient(
            null,
            array(
               'location' => self::$soap_server_address, // адрес SOAP-сервера - из soap_client_settings
   ...
   public static function login( $login, $pass ){
      ...
      try {
         // Логинимся
         // $_POST['password'] = iconv('KOI8-R', 'UTF-8', $_POST['password']);
         // $loginresult = $client->logIn($_POST['login'], $_POST['password']);
         $loginresult = self::$client->logIn($login, $pass);
      } catch (SoapFault $fault) {
      ...
      if ($loginresult->status->code == '0') {
         ...
         self::$client->__setCookie('SOAPClient', $loginresult->status->message);
   ...
   function a3()
   {
      if ((isset($_POST['login']) && isset($_POST['password'])) && (!empty($_POST['login']) && !empty($_POST['password']))) {
         ...
         try {
            // вызываем функцию проверки существования домена 
            $_POST['domain'] = iconv('KOI8-R', 'UTF-8', $_POST['domain']);
            $check_domain = $client->checkDomainAvailable($_POST['domain']);
         } catch (SoapFault $fault) {
            // Не удалось вызвать функцию checkDomainAvailable на сервере или она отработала неправильно.
            echo $format1 . "Couldnt execute checkDomainAvailable" . $format2;
            echo "Fault code: " . $fault->faultcode . "<br>Fault message: " . $fault->faultstring;
            logout();
            exit();
         }
         if ($check_domain->status->code != '1') {
            // функция отработала, возникла обработанная ошибка с идентификатором $check_domain->status->name, выдаем сообщение.
            echo $format1 . "Failed to check domain information " . $format2;
            echo "Error name: " . $check_domain->status->name . " <br>Error message: " . $check_domain->status->message;
            echo "<br>Error available: " . $check_domain->available;
            logout();
            exit();
         } else {
            // Успешно проверили "доступность".
            echo "<br><b>checkDomainAvailable status:</b> " . $check_domain->status->name;
            echo "<br><b>checkDomainAvailable message:</b> " . $check_domain->status->message;
            echo "<br><b>checkDomainAvailable available:</b> " . $check_domain->available;
         }
         logout();
      } else {
