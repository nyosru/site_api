...
if ( (isset($_POST['login']) && isset($_POST['password'])) && (!empty($_POST['login']) && !empty($_POST['password'])) )
{
   ...
   if (isset($_POST['domain']))
   {
      ...
      {         
         //**
         $getmyauctiondomains_result = $client->getMyAuctionDomains($params,$_POST['strict'],$_POST['sort_field'],$_POST['sort_dir'],
                                                  $_POST['limit'],$_POST['pagenum']);
          
      }
      catch(SoapFault $fault)
      {
         // Не удалось вызвать функцию getMyAuctionDomains на сервере или она отработала неправильно.
         echo $format1."Couldnt execute getMyAuctionDomains".$format2;
         echo "Fault code: ".$fault->faultcode."<br>Fault message: ".$fault->faultstring;
         logout();
         exit();
      }
      if ($getmyauctiondomains_result->status->code != '1')
      {
         // функция отработала, возникла обработанная ошибка с идентификатором $getMyAuctionDomains_result->status->name, выдаем сообщение.
         echo $format1."Failed to get auction domain list.".$format2;
         echo "Error name: ".$getmyauctiondomains_result->status->name." <br>Error message: ".$getmyauctiondomains_result->status->message;
         logout();
         exit();
      }
      else
      {
         // Успешно 
         echo "<br><b>getMyAuctionDomains status ID:</b> ".$getmyauctiondomains_result->status->name;          
         echo "<br><b>getMyAuctionDomains message:</b> ".$getmyauctiondomains_result->status->message;
         echo "<br><b>total domains found:</b> ".$getmyauctiondomains_result->data->listinfo->total;          
         echo "<br><b>total pages:</b> ".$getmyauctiondomains_result->data->listinfo->totalpages;          
         echo "<br><b>current limit:</b> ".$getmyauctiondomains_result->data->listinfo->limit;          
         echo "<br><b>current page:</b> ".$getmyauctiondomains_result->data->listinfo->page;          
         echo "<table border=1><tr><td>N</td><td>name</td><td>tyc</td><td>tyc is mirror</td><td>pr</td><td>state</td>
               <td>state name</td><td>start time</td><td>finish time</td><td>time to finish (timestamp)</td>
               <td>formatted time to finish</td><td>time to pay (timestamp)</td><td>formatted time to pay</td>
               <td>price</td><td>next bid</td><td>participant count</td><td>first reg date</td></tr>";
         foreach($getmyauctiondomains_result->data->au_domainarray as $key => $domain)
         {
            echo "<tr><td>".($key+1)."</td><td>".$domain->name."</td><td>".$domain->tyc."</td><td>".$domain->tyc_ismirror."</td><td>"
                ...
                .$domain->time_to_pay."</td><td>".$domain->format_time_to_pay."</td><td>".$domain->price.' '.$domain->currency."</td><td>"
                .$domain->next_bid."</td><td>".$domain->participant_count."</td><td>".$domain->first_reg_date."</td></tr>";
         }
         echo '</table>';
      }
   }
